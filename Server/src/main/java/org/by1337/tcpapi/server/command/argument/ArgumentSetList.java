...
public class ArgumentSetList<T> extends Argument<T> {
    ...
    public Object process(T sender, String str) throws CommandSyntaxError {
        ...
        if (!list.contains(str)) {
            if (list.size() > 10)
                throw new CommandSyntaxError("'%s' - invalid option! All possible values for %s: %s, and others...", str, list.subList(0, 5), list.size() - 10);
            throw new CommandSyntaxError("'%s' - invalid option! All possible values: %s:", str, items.get());
        }
        return str;
    }
